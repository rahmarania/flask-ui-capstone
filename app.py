# hanya bisa dirun menggunakan flask run
...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(["Category","App"]).agg({"Reviews":"sum","Rating":"mean"}).sort_values(by="Reviews", ascending=False).reset_index().iloc[0:10].to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    fig.plot(kind='bar',figsize=(20,20))
    ...
    plt.savefig('pricecat.png',bbox_inches="tight")
